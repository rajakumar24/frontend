...
class App extends React.Component {
    ...
    _verifyCode = async () => {      
        ...
        await axios.get("http://localhost:3001/api/profile/verify", {
            ...
            .then(data => {console.log("ook", data)
            ...
            <div style={{
               ...
               {/* <h3>By RpClan</h3> */}
                ...
                <Paper elevation={4} style={{ padding: 20, width: 300, marginBottom: 60 }}>
                    {!this.state.otpShow ? <h3 style={{ marginLeft: 10, color: '#9f9f9f' }}>GetRightProperty</h3> : <IconButton onClick={() => {
                    ...
                    <div>
                        {!this.state.otpShow ? <div style={{ display: 'flex', flexDirection: 'row', marginLeft: 'auto', justifyContent: 'space-around' }}>
                            ...
                            <div style={{ alignItems: 'flex-end', justifyContent: 'center', display: 'flex', marginRight: 10, width: 60 }}>
                                <TextField id="countryCode" label="Code" color="secondary" value={this.state.countryCodeValue} onChange={e =>
                                    this.setState({ countryCodeValue: e.target.value })
                                } />
                            </div>
                            <div>
                                <TextField id="phone" label="phone" color="secondary" value={this.state.phone}
                                    onChange={e => {
                            ...
                            <TextField id="token" label="otp" color="secondary" value={this.state.token} onChange={e =>
